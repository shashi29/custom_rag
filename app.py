...
from qdrant_client.http.models import PointStruct, Filter, FieldCondition, MatchValue, UpdateStatus, models
from qdrant_client import QdrantClient
from qdrant_client.models import PointStruct, VectorParams, Distance, UpdateStatus, OptimizersConfigDiff
...
from fastapi import FastAPI, UploadFile, File, Form, HTTPException, Depends
from pydantic import BaseModel
from sqlmodel import SQLModel, Field, Column, VARCHAR, TEXT, create_engine, Session, select
from qdrant_client import QdrantClient
from sentence_transformers import SentenceTransformer
from langchain.llms import OpenAI
from langchain.chains import ConversationChain
...
class Collection(SQLModel, table=True):
    id: int = Field(default=None, primary_key=True)
    name: str = Field(sa_column=Column(VARCHAR(255), nullable=False, unique=True))
...
class ChatHistory(SQLModel, table=True):
    ...
    sender: str = Field(sa_column=Column(VARCHAR(50), nullable=False))
    message: str = Field(sa_column=Column(TEXT, nullable=False))
    collection_id: int = Field(default=None, sa_column=Column(Integer, ForeignKey('collection.id', ondelete='CASCADE')))
    timestamp: datetime = Field(default_factory=datetime.utcnow)
...
async def create_collection(
    ...
    try:
        ...
        collections = qdrant_client.get_collections()
        if any(collection.name == collection_name for collection in collections.collections):
            raise HTTPException(status_code=400, detail=f"Collection '{collection_name}' already exists in Qdrant")
...
@app.get("/collections", response_model=List[CollectionInfo], description="Retrieve all collections")
async def get_all_collections(
    db: Session = Depends(get_session),
    limit: int = Query(100, description="Number of collections to retrieve"),
    offset: int = Query(0, description="Offset for pagination")
):
    collections = db.query(Collection).offset(offset).limit(limit).all()
    return [CollectionInfo(
        id=col.id,
        name=col.name,
        created_date=col.created_date,
        updated_date=col.updated_date
    ) for col in collections]
    
# Delete Collection by ID
@app.delete("/collections/{collection_id}", description="Delete a collection by its ID")
async def delete_collection_by_id(
    ...
    try:
        ...
        db.query(ChatHistory).filter(ChatHistory.collection_id == collection_id).delete()
        ...
        return {"message": f"Collection '{collection.name}' (ID: {collection_id}) deleted successfully"}
    except Exception as e:
        db.rollback()
...
async def submit_job(
    ...
    try:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        object_name = f"{collection.name}/{timestamp}_{file.filename}"
        bucket_name = "documents"
...
async def chat(
    ...
    # Use collection.name for Qdrant collection name
    collection_name = collection.name
...
async def get_chat_stats(
    ...
    query = db.query(ChatHistory).filter(ChatHistory.collection_id == collection_id)
